...
const Timeline = () => {
    ...
    const [isIITGN, setIsIITGN] = useState(false);

    const currentSchedule = isIITGN ? iitgnSchedule : schedule;
                ...
                <div className="rounded-[6rem] mt-24 border-[2px] border-black bg-[#D3AEFF] w-full ssm:w-[calc(100vw-4rem)]   ssm:max-w-7xl px-[clamp(1rem,4vw,2.44rem)] pt-[clamp(2rem,3.51rem,5vw)] pb-[clamp(2rem,3.51rem,5vw)]">
                    <div className="rounded-[6rem] border border-black bg-[#FAF5FF] w-full pt-8 px-6 pb-6 relative overflow-hidden">
                        ...
                        <div className="flex justify-end mb-4">
                            <button
                                onClick={() => setIsIITGN((prev) => !prev)}
                                className={`px-6 font-archivo py-2 rounded-full font-bold border-2 border-black transition-all duration-300 ${
                                    isIITGN
                                        ? "bg-[#FFD32B] text-black"
                                        : "bg-white text-black"
                                }`}
                            >
                                {isIITGN ? "Online Schedule" : "IITGN Schedule"}
                            </button>
                        </div>
